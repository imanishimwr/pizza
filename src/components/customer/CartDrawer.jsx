...
export default function CartDrawer({ isOpen, onClose, cart, onUpdateQty, onRemoveItem, onProceedCheckout }) {
  ...
  const handleApplyVoucher = (e) => {
    ...
    } else if (voucher.toUpperCase() === 'BOGOPIZZA') {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/75 backdrop-blur-sm animate-fade-in">
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
        <div className="w-full sm:w-screen sm:max-w-md bg-surface-dark border-l border-white/10 shadow-2xl flex flex-col justify-between">
          ...
          <div className="p-5 overflow-y-auto space-y-4 flex-1">
            {cart.length === 0 ? (
              ...
              cart.map((item, index) => (
                <div 
                  key={index}
                  className="p-3.5 rounded-xl bg-surface-card border border-white/5 space-y-2 relative group"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex gap-3">
                      <img
                        ...
                        onError={(e) => { e.target.src = item.meal.fallbackImage; }}
                      />
                      <div>
                        ...
                        {item.specialNote && (
                          <span className="text-[10px] text-text-subdued italic block">
                            "{item.specialNote}"
                          </span>
                        )}
                      </div>
                    ...
                    <button
                      ...
                      className="text-text-subdued hover:text-red-400 transition-colors p-1"
                      title="Remove item"
                    >
                  ...
                  <div className="pt-2 border-t border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-2 bg-black/40 border border-white/10 rounded-lg p-0.5">
                      <button
                        ...
                        className="w-6 h-6 rounded bg-surface-dark text-text-main hover:bg-white/10 flex items-center justify-center font-bold text-xs"
                      >
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="w-5 text-center font-bold text-xs font-mono text-white">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => onUpdateQty(index, item.quantity + 1)}
            ...
            <div className="p-5 bg-surface-card border-t border-white/10 space-y-4">
              ...
              {appliedVoucher && (
                <div className="p-2 rounded-lg bg-emerald-950/60 border border-emerald-500/40 text-emerald-400 text-xs font-semibold flex items-center justify-between">
                  ...
                  <button onClick={() => setAppliedVoucher(null)} className="text-[10px] underline hover:text-white">
                    Remove
                  </button>
                </div>
              )}
